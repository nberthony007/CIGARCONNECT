...
export const TradeDiptychCard: React.FC<TradeDiptychCardProps> = ({
  ...
  return (
    <>
      <article className="bg-[#FFFFFF] rounded-3xl p-6 sm:p-8 border border-cigar-stone shadow-xs space-y-6 h-full transition-colors">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-11 gap-4 sm:gap-6 items-center">
          ...
          <div 
            ...
            onKeyDown={(e) => {
              if (e.key === "Enter" || e.key === " ") {
                e.preventDefault();
                handleInspectCigar(offeredCigar, "offered");
              }
            }}
            className="lg:col-span-5 bg-[#FAF8F5]/60 hover:bg-white p-4 sm:p-5 rounded-2xl border border-cigar-stone/90 hover:border-cigar-brass transition-all duration-200 cursor-pointer shadow-xs hover:shadow-md hover:-translate-y-0.5 active:scale-[0.99] group relative overflow-hidden focus:outline-none focus-visible:ring-2 focus-visible:ring-cigar-brass"
            title="Cliquer pour examiner la fiche complète de cette vitole"
          >
            {/* Badge d'interaction au hover */}
            <div className="absolute top-2.5 right-2.5 opacity-0 group-hover:opacity-100 transition-opacity duration-200 flex items-center gap-1 text-[10px] font-bold text-cigar-cedar bg-[#F4F0E7] px-2 py-0.5 rounded-full border border-cigar-brass/40 shadow-xs pointer-events-none">
              <Eye className="w-3 h-3 text-cigar-brass" />
              <span>Voir détails</span>
            </div>

            <div className="flex items-center gap-4">
              <div className="relative shrink-0 overflow-hidden rounded-xl border border-cigar-stone shadow-xs">
                <img
              ...
              <div className="space-y-1 min-w-0 flex-1">
                ...
                {/* Bouton d'action rapide intégré */}
                <div className="pt-2 flex items-center gap-2">
                  <span className="inline-flex items-center gap-1 text-[11px] font-semibold text-cigar-brass group-hover:text-cigar-cedar transition-colors">
          ...
          <div className="lg:col-span-1 flex flex-col items-center justify-center py-2 lg:py-0">
            <button 
              ...
              className="w-11 h-11 rounded-full bg-cigar-ivory border border-cigar-brass/60 hover:border-cigar-cedar hover:bg-[#FAF7F2] hover:scale-105 active:scale-95 flex items-center justify-center text-cigar-cedar shadow-xs cursor-pointer transition-all duration-200 group focus:outline-none focus-visible:ring-2 focus-visible:ring-cigar-brass"
              title="Modifier les vitoles ou soumettre une contre-offre"
              aria-label={`Modifier les vitoles de l'échange V${activeVersion.versionNumber}`}
            >
          ...
          <div 
            ...
            onKeyDown={(e) => {
              if (e.key === "Enter" || e.key === " ") {
                e.preventDefault();
                handleInspectCigar(desiredCigar, "desired");
              }
            }}
            className="lg:col-span-5 bg-[#FAF8F5]/60 hover:bg-white p-4 sm:p-5 rounded-2xl border border-cigar-stone/90 hover:border-cigar-brass transition-all duration-200 cursor-pointer shadow-xs hover:shadow-md hover:-translate-y-0.5 active:scale-[0.99] group relative overflow-hidden focus:outline-none focus-visible:ring-2 focus-visible:ring-cigar-brass"
            title="Cliquer pour examiner la fiche complète de cette vitole"
