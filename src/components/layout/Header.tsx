...
export const Header: React.FC = () => {
  ...
  return (
    <>
      {/* Bandeau d'information sobre — Phase Pilote (défile naturellement en haut de page) */}
      <aside 
        aria-label="Annonce Phase Pilote"
        className="w-full bg-[#211D19] text-[#F7F5F0] flex items-center justify-center gap-2 px-3 py-1.5 text-[11px] font-normal tracking-wide border-b border-[#3D352E]/40 relative z-30"
      >
        <span className="inline-flex items-center gap-1 font-medium text-[#A88958] shrink-0">
          <ShieldCheck className="w-3.5 h-3.5 text-[#A88958]" strokeWidth={2} />
          Phase Pilote
        </span>
        <span className="text-[#857A6D] hidden sm:inline">·</span>
        <span className="text-[#EEEAE3] truncate text-[10px] sm:text-[11px]">
          Where Cigars Connect — Inventaires privés &amp; échanges de gré à gré.
        </span>
      </aside>

      {/* Barre de navigation principale STICKY — Ancrée parfaitement à top-0 sans déborder ni couper le logo */}
      <header className="sticky top-0 z-50 w-full site-header cigar-glass bg-[#F7F5F0]/95 backdrop-blur-md border-b border-[#D9D2C7] transition-all">
        <div className="max-w-7xl mx-auto px-3 sm:px-6 lg:px-8 h-14 sm:h-16 md:h-20 flex items-center justify-between gap-2 sm:gap-4">
          {/* Logo officiel CigarConnect (retour accueil public) — dimensionnement réactif parfait */}
          <Link 
            href="/" 
            onClick={(e) => {
          ...
          >
            <img
              ...
              className="h-8 sm:h-10 md:h-12 w-auto object-contain transition-opacity hover:opacity-90"
            />
          </Link>
          ...
          {/* Actions à droite : Recherche, Notifications & Compte / Menu */}
